...
function SingleDrink() {
  ...
  useEffect(() => {
    ...
    .then((data)=>{
      setSingleDrink(data.drinks.find((each)=> each.drink_url == id))
    })
  )
  }, []);
  ...
  if (!singleDrink) {
    ...
    const {drink_id,drink_name,drink_url,drink_brief_description,drink_price,drink_image_url,drink_description} = singleDrink
    return (
      <>
        <div className="food_card_container flex justify-center items-center h-fit! ">
          <div className="singleDrinks_content w-96 p-10 lg:w-10/12! lg:flex lg:justify-between">
            ...
            <div className="text_contents mt-5 h-fit lg:w-2/5!">
              <h2 className="text-2xl font-bold uppercase h-15 mb-2">{drink_name}</h2>
              <p className="text-[#4d4747] h-15 text-[20px ]  mb-2">{drink_brief_description}</p>
